...
export default function Steps() {
  ...
  const validateUsername = (value) => {
    ...
    if (value.includes(" ")) {
      ...
      setUsernameError(
        "Username cannot contain special characters except underscore and dash"
      );
    } else if (value === "") {
  ...
  const handleToggleAll = (optionsArray, stateArray, setStateFn) => {
    const allSelected = optionsArray.every((option) =>
      stateArray.includes(option)
    );
  ...
  const handleSubmit = async () => {
    ...
    if (!isUsernameValid) {
      setUsernameError(
        "Please enter a valid username (min 5 characters, no spaces/symbols)."
      );
  ...
  return (
    ...
    >
      ...
      >
        {darkMode ? (
          <i className="fa-solid fa-sun fa-fade"></i>
        ) : (
          <i className="fa-solid fa-moon fa-fade"></i>
        )}
      </button>
      <section className="w-full text-white flex flex-col justify-between items-center">
        <div
          className={`container transition-colors duration-500 ${
            darkMode ? "bg-black text-white" : "bg-white text-black"
          }`}
        >
          <Stepper
          ...
          >
            <Step>
              <h2 className="font-bold">
                Welcome to <i>CoreFocus</i>!
              </h2>
              <p className="text-gray-500">
                Follow the profile setup to get started...
              </p>
            </Step>
            <Step>
              ...
              <input
                ...
                className="font-bold italic text-gray-300 mt-3 outline-none"
              />
              {usernameError && (
            ...
            <Step>
              <h2 className="font-bold w-90">
                Tell us more about what you want to achieve using CoreFocus?
              </h2>
              <p className="text-gray-500">Select all that apply</p>
              <ul className="flex flex-row flex-wrap gap-2 mt-2 transition-colors duration-500">
                {goalOptions.map((option) => {
                  ...
                  return (
                    <li
                      key={option}
                      onClick={() =>
                        handleToggleItem(option, achieve, setAchieve)
                      }
                      className={`cursor-pointer p-1 px-3 rounded-lg font-semibold transition-colors duration-300 ease-in
                            ${
                              darkMode
                ...
                <li
                  onClick={() =>
                    handleToggleAll(goalOptions, achieve, setAchieve)
                  }
                  className={`cursor-pointer p-1 px-3 rounded-lg font-semibold transition-colors duration-500 ease-in-out
                        ${
                          darkMode
            ...
            <Step>
              ...
              <p className="text-gray-500">
                If you had to write only one goal down, what would it be...
              </p>
              <input
                className="text-gray-300 font-bold italic mt-3 outline-none"
                value={overallGoal}
            ...
            <Step>
              <span>
                <h2 className="inline font-bold mr-2">
                  Congrats, you&apos;re one step closer to achieving your goals{" "}
                </h2>
